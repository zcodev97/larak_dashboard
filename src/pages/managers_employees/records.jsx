...
function UsersUnderManagersPage() {
  ...
  async function loadData(page = 1) {
    setLoading(true);
    await fetch(
      SYSTEM_URL + `users/get-clients-managers-employee/?page=${page}`,
      {
        method: "GET",
        headers: {
    ...
    <>
      ...
      <div className="container">
        <p style={{ fontSize: "16px", fontWeight: "bold" }}>
          {data.count} {data.count > 1 ? "Records" : "Record"}
        </p>
      </div>
                ...
                <tbody>
                  {paginatedData.map((item) => (
                    <tr key={item.vendor_id + Math.random() * 10}>
                      ...
                      <td>{item.supervisor_first_name}</td>

                      <td>
                        {new Date(item.date_joined).toISOString().slice(0, 10)}
                      </td>
...
export default UsersUnderManagersPage;
